...
### Nicole's Summary/Changes based on Peer Comments
1. I am using the yeti theme which is in base r so I am adding a table of contents to add something interesting to my YAML per Erin's suggestion.

2. While reading through Reese's code, I saw a method for challenge 2 that I thought solved some of my clunkiness. Originally I was extracting rows and columns like this from the matrix: x <- c(m[, 2], m[, 3], m[, 6]) but she did it like this: x <- m[, c(2, 3, 6)]. This is just a little easier to right out and less redundant. I have edited my own to reflect this. I also changed the variables so they don't get overwritten every time as Erin pointed out and I could run them each again if needed. For the last part of this challenge, changing the code changed my output. The instructions are: extract the elements from row 2, column 2 to row 6, column 9. My first attempt (retained for documentation) is c(m[2, ], m[, 2:6], m[, 9]) which pulls each row and column and lists ALL elements. My second attempt, m[2, c(2:6, 9)], only pulls the elements at row 2 and each column coordinate which is far fewer elements than the first. It's hard for me to know which one is more correct based on the instructions so I am leaving both because they are both useful for different applications.

3. Another thing I liked from Reese's code was using $ subsetting notation when coercing the matrix to a data frame in Challenge 5. In later modules, I have found it a lot easier to use $ when subsetting datasets to the column and I am adding in an example of this in my code so I can reference back to it.
...
z <- c(m[2, ], m[, 2:6], m[, 9]) #original
z
z1 <- m[2, c(2:6, 9)] #modified
z1
```
...
Subsetting this array requires 4 coordinates to get a single position. Each one corresponds to a location/dimension. It's easiest to work backwards from the farthest right dimension working left to figure out where it is finding the result.
